...
namespace LibraryBarcodeSystem
{
...

    {
        ...
        public void getuserdetails()
        {
            ...
            SqlCommand cmd = new SqlCommand("Select [Name], [AcademicYear], [Type], [Barcode], [Faculty], [Program], [Date], [InTime], [OutTime], [Status] from [InOutInfo] WHERE Date='" +dateValue+"' ORDER BY [ID] DESC" , con);
            ...
            gridCurrentUser.DataSource = dtInOut;
        }
        public void getuserdetails1()
        {
            SqlCommand cmd1 = new SqlCommand("Select [Name], [AcademicYear], [Type], [Barcode], [Faculty], [Program], [Date], [InTime], [OutTime], [Status] from [InOutInfo] WHERE Date='" + dateValue + "' ORDER BY [ID] DESC", con);
        ...
        private void BarcodeScanner_BarcodeScanned(object sender, BarcodeScannerEventArgs e)
        {
            ...
            else
            {
                ...
                speakModule("No such user exists");
        ...
        private void btnInOut_Click(object sender, EventArgs e)
        {
            ...
            String barcodetext = txtMbarcode.Text;
            txtBarcode.Text = barcodetext;
            cmd = new SqlCommand("Select [RollNo], [Name], [AcademicYear], [College], [Faculty], [Program], [Status], [Type] from [LibraryUserInfo] WHERE AcademicYear = '" + Program.AcademicYear + "' and Barcode='" + txtMbarcode.Text + "'", con);
            ...
            if (dtLibrary.Rows.Count == 1)
            {
                ...
                else
                {
                    ...
                    cmd.Parameters.AddWithValue("@Name", dtLibrary.Rows[0]["Name"].ToString());
                    cmd.Parameters.AddWithValue("@AcademicYear", dtLibrary.Rows[0]["AcademicYear"].ToString());
                    cmd.Parameters.AddWithValue("@Barcode", txtMbarcode.Text);
                    cmd.Parameters.AddWithValue("@Type", "Internal Student");
                    cmd.Parameters.AddWithValue("@Faculty", dtLibrary.Rows[0]["Faculty"].ToString());
                    cmd.Parameters.AddWithValue("@Program", dtLibrary.Rows[0]["Program"].ToString());
                    cmd.Parameters.AddWithValue("@Date", DateTime.Now.Date);
        ...
        {
            /*if (con.State == ConnectionState.Closed) con.Open();
            cmd = new SqlCommand("UPDATE InOutInfo SET Status='Out'  WHERE Name='" +textBox1.Text+ "' AND Status='In'", con);
            cmd.ExecuteNonQuery();
            cmd1 = new SqlCommand("UPDATE InOutInfo SET OutTime ='"+DateTime.Now.TimeOfDay+"'  WHERE Name='" + textBox1.Text + "' AND OutTime='00:00:00'", con);
            cmd1.ExecuteNonQuery();
            ResetFormControls();
            getuserdetails1();
            MessageBox.Show("Thank you......", "Saved", MessageBoxButtons.OK, MessageBoxIcon.Information);

            if (con.State == ConnectionState.Open) con.Close();*/
        }

        private void gridCurrentUser_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private void gridMscan_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            txtMbarcode.Focus();
        }

        public void speakModule(String message)
        {
